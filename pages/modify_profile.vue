  ...
  <div class="flex flex-col md:flex-row">
    ...
    <div class="w-full md:w-1/4 h-auto md:h-screen bg-[color-mix(in oklch,currentColor 35%,#0000)] p-4">
      <ul class="menu bg-base-200 p-4 rounded-box">
        ...
        <li>
          <a @click="activeForm = 'name'">{{
            $t("modifyProfiles.changeName")
          }}</a>
        </li>
        <li>
          <a @click="activeForm = 'picture'">{{
            $t("modifyProfiles.changePicture")
          }}</a>
        </li>
      </ul>
    </div>
...
onMounted(async () => {
  ...
  try {
    ...
    if (
      ...
      response.providers.includes("credentials")
    ) {
...
async function updatePicture() {
  $eventBus.on(
    "UpdatePicture",
    (payload: { picture: string; email: string }) => {
