  ...
  <div class="container mx-auto p-4">
    ...
    <div class="mb-4">
      <input
        ...
        type="text"
        :placeholder="$t('searchMovies.searchInput')"
    ...
    <div class="flex flex-wrap gap-4 mb-8">
      ...
      <select v-model="sortBy" class="select select-bordered">
        ...
        <option value="vote_average">
          {{ $t("searchMovies.sortRating") }}
        </option>
        <option value="release_date">
          {{ $t("searchMovies.sortReleaseYear") }}
        </option>
        <option value="popularity">
          {{ $t("searchMovies.sortPopularity") }}
        </option>
      </select>
      ...
      <input
        v-model="yearFilter"
        type="number"
        :placeholder="$t('searchMovies.filterYear')"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
      <div
        ...
        :class="[userWatchedMovies.includes(movie.imdb_id) ? 'opacity-40' : '']"
        @click="sendToPlayer(movie)"
      >
        <figure class="relative">
          <div
            v-if="userWatchedMovies.includes(movie.imdb_id)"
    ...
    <div
      v-if="filteredMovies.length === 0 && !loading"
    ...
    <div
      v-if="totalPages === page && filteredMovies.length > 0"
      class="flex justify-center my-8"
    >
      <p class="text-base line-clamp-3">
        {{ $t("searchMovies.stateNoMoreMovies") }}
      </p>
    </div>
  </div>
</template>
...
onMounted(async () => {
  // Fetch setup data
  try {
    const resGenres = await fetch(`/api/movies/genres`);
...
const searchMovies = async () => {
  try {
    ...
    if (searchQuery.value) {
      ...
      response = await fetch(`/api/movies` + `?page=${page.value}`);
    }
...
const handleScroll = () => {
  ...
  if (!bottomOfWindow || loading.value || totalPages.value == page.value)
    return;
