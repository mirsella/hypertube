...
async function check_credidentials_provider(email: string) {
  ...
  const userWithLocalProvider = await db
    ...
    .where(
      and(
        eq(tables.providers.email, email),
        eq(tables.providers.provider, "credentials"),
      ),
    );
...
export default defineEventHandler(async (event) => {
  ...
  if ((await check_credidentials_provider(email)) === false)
    return new Response("User doesn't have provider credidentials", {
      status: 400,
    });
